...
class TokenizePreprocessor(BaseEstimator, TransformerMixin):
    ...
    def token_representation(self, sentence):
        return [x for x in list(self.tokenize(sentence)) if x]
    ...
    def tokenize(self, sentence):
        ...
        if len(sentence) < MIN_LEN:
            ...
            for token in self.tokenizer.tokenize(sentence):
                ...
                subtokens = [self.normalize_token(t) for t in self.splitters.split(token_nrm) if t]
                ...
                transformer_list=[
                    ...
                    Pipeline([
                        ...
                        ("word_tfidf", TfidfVectorizer(
                                ...
                                binary=binary,
                                norm='l2' if not binary else None,
                                # use_idf=not binary
                            )
                         )
                    ])),
